...
public class TestTail extends HadoopTestCase {
  ...
  public void testTail_2() throws IOException {
    ...
    Tail.run(inputPath, outputPath, 2, conf);
  ...
  public void testTail_1() throws IOException {
    ...
    Tail.run(inputPath, outputPath, 1, conf);
    ReadVectors.run(outputPath.toString(), vectorMap, true);
    assertEquals("Number of rows is not equal to 1", 1, vectorMap.size());
  }
  
  @Test
  public void testTail_ExceedsSize() throws IOException {
    System.out.println("Exceeds Size, but still prints out all that it did find");
    Tail.run(inputPath, outputPath, 3, conf);
    ReadVectors.run(outputPath.toString(), vectorMap, true);
    assertEquals("Number of rows is not equal to 2", 2, vectorMap.size());
  }
  @Test
  public void testTail_Directory() throws IOException {
    assertEquals(1,Tail.run(tmpDir, outputPath, 2, conf));
  }
}
