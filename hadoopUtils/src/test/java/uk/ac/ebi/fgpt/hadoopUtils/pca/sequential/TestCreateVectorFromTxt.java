...
public class TestCreateVectorFromTxt extends HadoopTestCase {
  private String[][] data = new String[][] { {"2", "4", "6"}, {"8", "10", "12"}};
  private Path tmpDir;
  ...
  public void testCreateVector_NoSkip() throws IOException {
    CreateVectorFromTxt.main((new String[] {"-i", inputPath.toString(), "-o", outputPath.toString()}));
  ...
  public void testCreateVector_SkipOneRow() throws IOException {
    CreateVectorFromTxt.main((new String[] {"-i", inputPath.toString(), "-o", outputPath.toString(), "-hr",
                                            "1"}));
    System.out.println("A 1x3 Matrix is Below");
    ReadVectors.run(outputPath.toString(), vectorMap, true);
    assertEquals("Rows not equal to 1", 1, vectorMap.size());
    assertEquals("Column Cardinality not equal to 3", 3, vectorMap.firstEntry().getValue().get().size());
  }
  
  @Test
  public void testCreateVector_SkipOneColumn() throws IOException {
    CreateVectorFromTxt.main((new String[] {"-i", inputPath.toString(), "-o", outputPath.toString(), "-hc",
                                            "1"}));
    System.out.println("A 2x2 Matrix is Below");
    ReadVectors.run(outputPath.toString(), vectorMap, true);
    assertEquals("Rows not equal to 2", 2, vectorMap.size());
    assertEquals("Column Cardinality not equal to 2", 2, vectorMap.firstEntry().getValue().get().size());
  }
}
