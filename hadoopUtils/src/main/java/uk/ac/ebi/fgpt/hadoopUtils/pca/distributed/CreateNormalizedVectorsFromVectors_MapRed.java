 ...
 * keyvalue pairs. arg[0] is the input file arg[1] is the output file
 * 
 * @author vincent@ebi.ac.uk
 * 
 */
public class CreateNormalizedVectorsFromVectors_MapRed extends Configured
		implements Tool {

	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(),
				new CreateNormalizedVectorsFromVectors_MapRed(), args);
		System.exit(res);
	}
	...
	public static class Map extends
		...
		protected void map(
			...
			context.write(key, new VectorWritable(VectorOperations
					.normalize(value.get())));
		}
	}
	...
	public int run(String[] args) throws Exception {
		...
		if (args.length <= 1) {
			formatter.printHelp(
					"Create Normalized Vector From Vectors Using Map Reduce",
					cliOptions, true);
			System.exit(0);
		}
	...
	public void run(String pathToInput, String pathToOutPut)
			throws IOException, InterruptedException, ClassNotFoundException {
		Job job = new Job(getConf());
		job.setJarByClass(CreateNormalizedVectorsFromVectors_MapRed.class);
		job.setJobName("Create Normalized Vector "
				+ "From Vectors Using Map Reduce");

		// Set Input and Output Paths
		FileInputFormat.setInputPaths(job, new Path(pathToInput));
		FileOutputFormat.setOutputPath(job, new Path(pathToOutPut));
