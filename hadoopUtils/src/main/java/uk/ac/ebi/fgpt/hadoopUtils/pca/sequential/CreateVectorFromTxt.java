...
/**
 * This class takes a text file and creates it into a sequence file.
 * 
...
public class CreateVectorFromTxt extends SequentialTool {
  ...
  public static void main(String[] args) throws IOException {
    ...
    Option headerRows = OptionBuilder.withArgName("num").hasArg().withLongOpt("headerRow").withDescription(
      "The given input has <num> header rows").create("hr");
    Option headerColumns = OptionBuilder.withArgName("num").hasArg().withLongOpt("headerColumn")
        .withDescription("The given input has <num> header columns").create("hc");
    ...
    try {
      ...
      int numHeaderColumns = cmd.hasOption("hc") ? Integer.parseInt(cmd.getOptionValue("hc")) : 0;
      int numHeaderRows = cmd.hasOption("hr") ? Integer.parseInt(cmd.getOptionValue("hr")) : 0;
    ...
    try {
      ...
      while ((line = reader.readLine()) != null) {
        if (rowsToSkip < headerRows) {
          ...
          writer.append(new IntWritable(index), new VectorWritable(StringToVector
              .convert(line, headerColumns)));
          index++;
        }
      }
      writer.close();
