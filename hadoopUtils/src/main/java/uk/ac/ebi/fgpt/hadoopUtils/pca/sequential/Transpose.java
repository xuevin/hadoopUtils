...
public class Transpose extends SequentialTool {
  public static void main(String[] args) throws IOException {
    ...
    Option rows = OptionBuilder.withArgName("num").hasArg().isRequired().withLongOpt("rows").withDescription(
      "The given input has <num> rows").create("r");
    Option columns = OptionBuilder.withArgName("num").hasArg().isRequired().withLongOpt("columns")
        .withDescription("The given input has <num> columns").create("c");
  ...
  public static void run(String stringToInput, String stringToOutput, int numRows, int numColumns) throws IOException {
    ...
    try {
      ...
      for (int i = 0; i < numColumns; i++) {
        ...
        Vector tranposedColumn = new SequentialAccessSparseVector(numRows);
        
        while (reader.next(key, value)) {
          tranposedColumn.set(key.get(), value.get().get(i));
        }
        writer.append(new IntWritable(i), new VectorWritable(tranposedColumn));
